# Mastery Track: Quantitative Finance

[![License: MIT](https://img.shields.io/badge/license-MIT-1B4332?style=flat-square&labelColor=0d1117)](./LICENSE) [![Mastery Track](https://img.shields.io/badge/Mastery%20Track-The%20Built%20World-2F5233?style=flat-square&labelColor=0d1117)](https://github.com/RoyPiring) [![Systems](https://img.shields.io/badge/systems-0-2F5233?style=flat-square&labelColor=0d1117)](./INDEX.md) [![Updated](https://img.shields.io/badge/updated-2026--07--23-264653?style=flat-square&labelColor=0d1117)](./INDEX.md)

> *A hands-on route from amateur to command in quantitative finance, one build at a time. A route to command, not a degree.*

I'm learning quantitative finance the way it actually sticks: building the field end to end, one system at a time, with AI as the multiplier that makes real depth possible on my own hours. Not chapters I skim and forget, but working command I can show, and it grows in the open with every build.

## Featured Systems

The builds that best show the field will sit here as they land. The first ones are on the way.

## What it covers

Quantitative finance and derivatives, algorithmic and machine-learning trading, portfolio optimization, and risk modeling.

**What it isn't.** Not a degree, and no claim past what each build proves. Managing or advising on other people's money is regulated (the Advisers Act, FINRA and Reg BI), and every build is a study or paper mock-up: not advice, not a solicitation, and not a track record. A backtest is not a prediction.

## Progress

| Field | Value |
|-------|-------|
| Status | 0 / 100 committed |

Start at rank 1 in `ideas/mastery-finance-100/mastery-finance-100.md`. Full catalog: [`INDEX.md`](./INDEX.md).
